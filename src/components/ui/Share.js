...
export default function Share({ title }) {
  ...
  const style = {
    btn: "cursor-pointer rounded-lg hover:bg-neutral-200 dark:hover:bg-neutral-800 px-2 py-2 flex flex-row justify-center gap-1 items-center focus:bg-blue-500 dark:focus:bg-blue-600 focus:text-white",
    text: "text-sm mt-1 opacity-75 font-medium",
    rndbtn:
      "rounded-full border border-neutral-200 dark:border-neutral-800 bg-neutral-200 dark:bg-neutral-800 focus:bg-blue-500 dark:focus:bg-blue-500 focus:text-white size-12 flex justify-center items-center",
  };

  return (
    <div className="max-w-md w-full mx-auto">
      <h1 className="font-medium text-center">分享这个页面</h1>

      <div className="flex flex-col gap-1 mt-2">
        <div className="flex flex-row gap-1 items-center">
          <div className="max-w-64 whitespace-nowrap text-base select-all rounded-lg bg-neutral-200 dark:bg-neutral-800 overflow-x-hidden py-2.5 px-2">
            {url}
          </div>
          <Tooltip content="通过链接分享">
            <button
              aria-label="通过链接分享"
              onClick={handleCopyLink}
              className={style.rndbtn}
            >
              {copied ? (
        ...
        <h1 className="font-medium opacity-50 text-center my-2 text-sm">或</h1>

        <div className="flex flex-row gap-4 justify-center items-center">
          <Tooltip content="通过腾讯 QQ 分享">
            <button
              aria-label="通过腾讯 QQ 分享"
