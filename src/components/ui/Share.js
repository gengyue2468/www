...
export default function Share({ title }) {
  ...
  const style = {
    btn: "cursor-pointer rounded-3xl bg-neutral-200 dark:bg-neutral-800 px-6 py-4 flex flex-col items-center justify-center focus:bg-black dark:focus:bg-white focus:text-white dark:focus:text-black transition-all duration-500 focus:ring-2 focus:ring-neutral-500 focus:ring-offset-2",
    text: "text-xs font-semibold opacity-50 mt-1",
  };
      ...
      <div className="flex flex-row justify-between gap-2 mt-4">
        <Tooltip content="通过链接分享">
          ...
          >
            ...
            <p className={style.text}>{copied ? "已复制！" : "复制链接"}</p>
          </button>
        </Tooltip>
