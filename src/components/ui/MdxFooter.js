import { FrownIcon, SmileIcon, Loader2Icon } from "lucide-react";
import { useEffect, useState } from "react";
import ReactMarkdown from 'react-markdown';
import MdxWrapper from "./MdxWrapper";

export default function MdxFooter({ mdxSource }) {
  const [summary, setSummary] = useState("");
  const [isLoading, setIsLoading] = useState(false);
  const [error, setError] = useState("");
  const [abortController, setAbortController] = useState(null);

  useEffect(() => {
    if (!mdxSource?.compiledSource) return;

    if (abortController) {
      abortController.abort();
    }

    const controller = new AbortController();
    setAbortController(controller);
    setSummary("");
    setError("");
    setIsLoading(true);

    const fetchSummary = async () => {
      try {
        const response = await fetch("/api/summarize", {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
          },
          body: JSON.stringify({ article: mdxSource.compiledSource }),
          signal: controller.signal,
        });

        if (!response.ok) {
          throw new Error(`HTTP error! status: ${response.status}`);
        }

        if (!response.body) {
          throw new Error("ReadableStream not supported in this browser.");
        }

        const reader = response.body.getReader();
        const decoder = new TextDecoder();
        let done = false;

        while (!done) {
          const { value, done: readerDone } = await reader.read();
          done = readerDone;

          if (value) {
            const chunk = decoder.decode(value, { stream: true });
            const lines = chunk.split("\n").filter((line) => line.trim());

            for (const line of lines) {
              const data = line.replace(/^data: /, "").trim();

              if (data === "[DONE]") {
                setIsLoading(false);
                return;
              }

              if (!data) continue;

              try {
                const parsed = JSON.parse(data);
                if (parsed.content) {
                  setSummary((prev) => prev + parsed.content);
                }
              } catch (parseErr) {
                console.warn(
                  "解析 SSE 数据失败：",
                  parseErr,
                  "原始数据：",
                  data
                );
              }
            }
          }
        }

        setIsLoading(false);
      } catch (err) {
        if (err.name === "AbortError") {
          console.log("请求已被中止");
          return;
        }
        const errMsg = err.message || "总结请求失败";
        setError(errMsg);
        setIsLoading(false);
      }
    };

    fetchSummary();

    return () => {
      controller.abort();
    };
  }, [mdxSource]);

  return (

      <div className="rounded-xl bg-neutral-200/50 dark:bg-neutral-800/50 px-4 py-6 mt-16">
        <h1 className="font-medium text-center !text-base flex flex-row gap-2 items-center justify-center !mt-4 mb-4">
          <span>AI 文章总结</span>
          <span className="text-sm text-neutral-500 flex flex-row gap-1">
            <span>Powered by</span>
            <RiDeepseekFill className="text-blue-600 size-5" />
          </span>
        </h1>

        {isLoading && (
          <div className="flex items-center justify-center py-8 text-neutral-500">
            <Loader2Icon className="size-5 animate-spin mr-2" />
            <span>生成总结中...</span>
          </div>
        )}

        {error && !isLoading && (
          <div className="flex items-center justify-center gap-2 py-4 text-red-500 dark:text-red-400">
            <FrownIcon className="size-5" />
            <span>{error}</span>
          </div>
        )}

        {summary && !isLoading && !error && (
          <MdxWrapper>
            <ReactMarkdown>{String(summary)}</ReactMarkdown>
          </MdxWrapper>
        )}

        {!summary && !isLoading && !error && (
          <div className="text-center text-neutral-500 py-4 text-sm">
            文章内容将在这里生成总结
          </div>
        )}
      </div>
  );
}

export function RiDeepseekFill(props) {
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      width="1em"
      height="1em"
      viewBox="0 0 24 24"
      {...props}
    >
      <path
        fill="currentColor"
        d="M23.75 4.927c-.245-.12-.34.108-.482.224c-.049.038-.09.087-.131.13c-.357.384-.773.634-1.315.604c-.796-.044-1.474.207-2.074.818c-.127-.754-.551-1.203-1.195-1.492c-.338-.15-.68-.3-.915-.626c-.165-.231-.21-.49-.293-.744c-.052-.153-.105-.31-.28-.337c-.192-.03-.266.13-.341.265c-.3.55-.416 1.158-.406 1.772c.027 1.382.608 2.482 1.762 3.266c.132.09.166.18.124.311c-.079.27-.172.531-.255.8c-.052.173-.13.211-.314.135A5.3 5.3 0 0 1 15.97 8.92c-.82-.797-1.563-1.677-2.489-2.366a11 11 0 0 0-.66-.454c-.944-.922.125-1.679.372-1.768c.259-.093.09-.416-.747-.412c-.835.004-1.6.285-2.574.659c-.143.057-.326.153-.446.13a9.2 9.2 0 0 0-2.763-.096c-1.806.203-3.25 1.06-4.31 2.525c-1.275 1.76-1.574 3.759-1.207 5.846c.385 2.197 1.502 4.019 3.22 5.442c1.78 1.474 3.83 2.197 6.169 2.058c1.42-.081 3.003-.273 4.786-1.789c.45.224.922.313 1.707.381c.603.057 1.184-.03 1.634-.123c.704-.15.655-.804.4-.926c-2.065-.966-1.612-.573-2.024-.89c1.05-1.248 2.632-2.544 3.25-6.741c.049-.334.007-.543 0-.814c-.003-.163.034-.228.22-.247a4 4 0 0 0 1.482-.457c1.338-.734 1.867-1.939 1.995-3.385c.019-.22-.004-.45-.236-.565m-11.652 13.01c-2.002-1.58-2.972-2.1-3.373-2.078c-.375.021-.308.452-.225.733c.086.277.198.468.356.711c.109.162.184.402-.108.58c-.645.403-1.766-.134-1.82-.16c-1.303-.77-2.394-1.79-3.163-3.182c-.741-1.342-1.172-2.78-1.243-4.315c-.02-.372.09-.503.456-.57a4.5 4.5 0 0 1 1.466-.037c2.043.3 3.782 1.218 5.24 2.67c.832.829 1.462 1.817 2.11 2.783c.69 1.027 1.432 2.004 2.377 2.804c.333.281.6.495.854.653c-.768.085-2.05.104-2.927-.592m.96-6.199a.294.294 0 1 1 .588 0a.294.294 0 0 1-.296.296a.29.29 0 0 1-.293-.296m2.98 1.537c-.192.078-.383.146-.566.154a1.2 1.2 0 0 1-.765-.245c-.262-.22-.45-.343-.53-.73a1.7 1.7 0 0 1 .016-.566c.068-.315-.008-.516-.228-.7c-.18-.15-.408-.19-.66-.19a.5.5 0 0 1-.244-.076c-.105-.053-.191-.184-.109-.345a1 1 0 0 1 .185-.201c.34-.195.734-.13 1.098.015c.337.139.592.393.959.752c.375.434.442.555.656.88c.168.256.323.518.428.818c.063.186-.02.34-.24.434"
      />
    </svg>
  );
}
